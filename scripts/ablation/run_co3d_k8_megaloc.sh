...
# CO3D eval using per-sequence graphs built with MegaLoc (Top-K=8, mutual=true), no soft bias.
...
OUTPUT_DIR=${2:-"results/all_runs/run_co3d_k8_megaloc"}
...
# Forward timing with prebuilt graph on kitchen images (for apples-to-apples runtime)
...
# Build per-sequence graphs with MegaLoc (auto backbone tries MegaLoc, else falls back)
