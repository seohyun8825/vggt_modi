...
# Allow FlexAttention on long sequences and enable optional debug
...
if [[ "$RUN_MODE" == "forward" || "$RUN_MODE" == "both" ]]; then
  ...
  mkdir -p "$OUT_FWD"
  echo "[RUN] Ours K=10 forward -> $OUT_FWD"
...
if [[ "$RUN_MODE" == "eval" || "$RUN_MODE" == "both" ]]; then
  ...
  mkdir -p "$OUT_EVAL"
  echo "[RUN] Ours K=10 CO3D eval -> $OUT_EVAL"
  python tools/run_ablation.py \
    --image_folder "$FWD_IMAGE_DIR" \
    --output_dir "$OUT_EVAL" \
    --mask_type topk \
