...
# Hard Top-K + Soft bias (mixed) using mask_type=soft with topk>0
...
OUTPUT_DIR=${2:-"results/softmix_k8"}
...
if [[ "$RUN_MODE" == "forward" || "$RUN_MODE" == "both" ]]; then
  ...
  mkdir -p "$OUT_FWD"
  echo "[RUN] SoftMix K=8 forward -> $OUT_FWD"
...
if [[ "$RUN_MODE" == "eval" || "$RUN_MODE" == "both" ]]; then
  ...
  mkdir -p "$OUT_EVAL"
  echo "[RUN] SoftMix K=8 CO3D eval -> $OUT_EVAL"
  python tools/run_ablation.py \
    --image_folder "$FWD_IMAGE_DIR" \
    --output_dir "$OUT_EVAL" \
    --mask_type soft \
